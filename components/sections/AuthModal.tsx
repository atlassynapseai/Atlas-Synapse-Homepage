...
function AuthModalInner() {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          redirectTo: mode === 'signup'
            ...
            : `${window.location.origin}/?scroll=dashboard`,
        },
      })
  ...
  return (
    <div className="max-w-md w-full mx-auto lg:mx-24">
      {/* Header */}
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold text-white mb-2">Create Account or Sign In</h2>
        <p className="text-slate-400">Access your Atlas Synapse workspace</p>
      </div>
      ...
      <div className="rounded-xl border border-white/10 bg-slate-900/60 p-8">
        ...
        <form onSubmit={handleEmailAuth} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full rounded-lg bg-gradient-to-r from-purple-400 to-pink-400 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50 transition-all hover:shadow-lg hover:shadow-purple-400/40 hover:scale-[1.02] active:scale-95"
          >
    ...
    <section className="py-20 px-4">
      <Suspense fallback={
        <div className="max-w-md mx-auto text-center">
          <div className="h-8 w-8 mx-auto animate-spin rounded-full border-2 border-purple-400 border-t-transparent" />
        </div>
      }>
        <AuthModalInner />
      </Suspense>
