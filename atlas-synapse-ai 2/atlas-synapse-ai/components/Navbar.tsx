...
export default function Navbar() {
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 min-h-[4rem] border-b transition-all duration-300 overflow-hidden ${
        scrolled ? "border-white/10 bg-atlas-bg/85 backdrop-blur-xl" : "border-transparent bg-transparent"
      }`}
      ...
      <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between gap-6 px-4 py-0 sm:px-6 lg:px-10">
        ...
        >
          <div className="relative h-8 w-8 shrink-0 overflow-hidden rounded-full bg-gradient-to-tr from-atlas-primary to-atlas-secondary shadow-atlas-glow-sm sm:h-9 sm:w-9">
            <Image src="/logo.png" alt="Atlas Synapse" fill sizes="36px" priority />
          </div>
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className={`relative flex h-10 items-center rounded-xl px-4 py-0 text-sm font-medium tracking-wide transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-atlas-primary sm:h-11 sm:px-5 sm:text-base ${
                  active ? "text-atlas-secondary" : "text-slate-300 hover:text-white"
                }`}
              >
                {active && (
                  <motion.span
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`rounded-xl px-4 py-3 text-sm font-medium ${
                    pathname === link.href ? "bg-atlas-primary/15 text-atlas-secondary" : "text-slate-300"
                  }`}
                >
                  {link.label}
                </Link>
